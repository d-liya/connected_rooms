...
import { loadGameApiClient } from "./sdk";
...
export const ACTIVE_GAME: GameDefinition = {
  ...
  assets: {
    ...source.assets,
    titleArt: asset(thumbnail),
    world: asset(source.assets.world),
    images: Object.fromEntries(
      Object.entries(source.assets.images).map(([id, url]) => [id, asset(url)]),
    ),
    characters: Object.fromEntries(
      Object.entries(source.assets.characters).map(([id, states]) => [
        id,
        Object.fromEntries(
          Object.entries(states).map(([state, sheet]) => [
            state,
            { ...sheet, src: asset(sheet.src) },
          ]),
        ),
      ]),
    ),
    audio: {
      ...source.assets.audio,
      music: {
        ...source.assets.audio.music,
        src: asset(source.assets.audio.music.src),
      },
      ambience: {
        ...source.assets.audio.ambience,
        src: asset(source.assets.audio.ambience.src),
      },
      secondaryLoop: source.assets.audio.secondaryLoop
        ? {
            ...source.assets.audio.secondaryLoop,
            src: asset(source.assets.audio.secondaryLoop.src),
          }
        : undefined,
      sfx: Object.fromEntries(
        Object.entries(source.assets.audio.sfx).map(([id, clip]) => [
          id,
          { ...clip, src: asset(clip.src) },
        ]),
      ),
      voices: {
        ...source.assets.audio.voices,
        root: asset(source.assets.audio.voices.root),
      },
    },
  },
};
...
// by default — set VITE_ANALYTICS_ENABLED=false to opt out. The client script
// itself loads through the shared loader in sdk.ts (single URL and script
// tag; VITE_GAME_API_CLIENT_URL overrides the pinned default there).
export function isGameAnalyticsEnabled(): boolean {
  const raw = (
    (import.meta.env.VITE_ANALYTICS_ENABLED as string | undefined) ?? ""
  )
    .trim()
    .toLowerCase();
  if (!raw) return true;
...
export function startGameAnalytics(): void {
  ...
  loadGameApiClient()
    .then(async () => {
      const Client = window.GameServerClient;
      if (!Client)
        throw new Error(
          "game API client did not register window.GameServerClient",
        );
      if (!sharedClient) {
        const baseUrl = (
    ...
    .catch((error) => {
      ...
      console.warn(
        "[analytics] gameplay analytics unavailable; continuing without it.",
        error,
      );
    });
}
